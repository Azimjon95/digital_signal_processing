...
env_name = "DGU01"
base_path = os.path.join("C:", "ProgramData", "miniconda3", "envs", env_name, "Lib", "site-packages")

# Ensure it works across platforms
if os.path.exists(base_path):
    site.addsitedir(base_path)  # Preferred over sys.path.append()
else:
...
def hanning_window(n, N_tochechnoe):
    return 1/2*(1-math.cos(2 * pi * n /N_tochechnoe))
...
for m in range(N_tochechnoe):
    ...
    for n in range(N_tochechnoe):
        xwin = hanning_window(n, N_tochechnoe) * A1 * math.sin(2 * pi * F1 * n * ts + RF1) + A2 * math.sin(2 * pi * F2 * n * ts + RF2)
        Xm += xwin * math.cos(2 *pi * chastotniy_analiz(n, m, N_tochechnoe)) - 1j * xwin * math.sin(2 * pi * chastotniy_analiz(n, m, N_tochechnoe))
...
mx = chastotniy_analiz_mx(16, 16, 16)
x = np.array(mx)
y = np.array(Xmagy)
plt.subplot(2, 2, 1)
plt.plot(x,y, 'o')
